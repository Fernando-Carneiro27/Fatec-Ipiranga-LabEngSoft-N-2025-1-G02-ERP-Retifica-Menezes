...
import {
  ...
  IconButton, Tooltip, Button, Dialog, DialogTitle, DialogContent, FormControlLabel, Checkbox,
...
const ListarClientes = () => {
  ...
  const formatarNomes = (coluna: string) => {
    const mapColunas = {
      nome: 'Nome', cpf_cnpj: 'CPF/CNPJ', email: 'Email', telefone: 'Telefone',
      bairro: 'Bairro', cep: 'CEP', endereco: 'Endereço', status_cliente: 'Status', observacao: 'Observação',
    };
  ...
  const handlePesquisar = () => {
    ...
    setClientesFiltrados(filtrados); setProcurado('');
  };
    ...
    <HelmetProvider>
      ...
      <Box>
        <Box sx={stylesLista.headerCliente}>
          ...
          />
          <Box sx={stylesLista.headerClienteSpan}>
            Clientes - Visualizar
          </Box>
        </Box>

        <Box sx={stylesLista.buttons}>
          <Button variant="contained" color="success" size="large" sx={{ textTransform: 'none', fontSize: '15px' }} onClick={handleAddCliente}>
        ...
        <Box sx={stylesLista.clientesList}>
          <Card sx={{ width: '100%', padding: '10px', boxShadow: 'none' }}>
            <TableContainer sx={stylesLista.tableContainer}>
              <Table size="medium" aria-label="clientes table" sx={{ border: '2px solid #959494', borderRadius: '20px' }}>
                <TableHead>
                  <TableRow sx={{ backgroundColor: '#DCDDDE', width: '100%', alignItems: 'center' }} >
                    { colunasVisiveis.nome && ( <TableCell sx={stylesLista.colunas}>NOME</TableCell> )}
                    { colunasVisiveis.cpf_cnpj && ( <TableCell sx={stylesLista.colunas}>CPF / CNPJ</TableCell> )}
                    { colunasVisiveis.email && ( <TableCell sx={stylesLista.colunas}>EMAIL</TableCell> )}
                    { colunasVisiveis.telefone && ( <TableCell sx={stylesLista.colunas}>TELEFONE</TableCell> )}
                    { colunasVisiveis.bairro && ( <TableCell sx={stylesLista.colunas}>BAIRRO</TableCell> )}
                    { colunasVisiveis.cep && ( <TableCell sx={stylesLista.colunas}>CEP</TableCell> )}
                    { colunasVisiveis.endereco && ( <TableCell sx={stylesLista.colunas}>ENDEREÇO</TableCell> )}
                    { colunasVisiveis.status_cliente && ( <TableCell sx={stylesLista.colunas}>STATUS</TableCell> )}
                    { colunasVisiveis.observacao && ( <TableCell sx={stylesLista.colunas}>OBSERVAÇÕES</TableCell> )}
                    <TableCell sx={stylesLista.colunas}>AÇÕES</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {(clientesFiltrados.length > 0 ? clientesFiltrados : clientes).length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={9} align="center">
                        <Typography gutterBottom>Nenhum cliente encontrado.</Typography>
                      </TableCell>
                    </TableRow>
                  ) : (
                    (clientesFiltrados.length > 0 ? clientesFiltrados : clientes).map((cliente) => (
                      <TableRow key={cliente.id}>
                        ...
                        {colunasVisiveis.cpf_cnpj && (
                          <TableCell sx={stylesLista.linhas}>
                            <Typography gutterBottom>{cliente.cpf_cnpj}</Typography>
                          </TableCell>
                        )}
                        {colunasVisiveis.email && (
                          <TableCell sx={stylesLista.linhas}>
                            <Typography gutterBottom>{cliente.email}</Typography>
                          </TableCell>
                        )}
                        {colunasVisiveis.telefone && (
                          <TableCell sx={stylesLista.linhas}>
                            <Typography gutterBottom>{cliente.telefone}</Typography>
                          </TableCell>
                        )}
                        ...
                        {colunasVisiveis.endereco && (
                          <TableCell sx={stylesLista.linhas}>
                            <Typography gutterBottom>{cliente.endereco}</Typography>
                          </TableCell>
                        )}
                        {colunasVisiveis.status_cliente && (
                          <TableCell sx={stylesLista.linhas}>
                            <Typography gutterBottom>{cliente.status_cliente}</Typography>
                          </TableCell>
                        )}
                        {colunasVisiveis.observacao && (
                          <TableCell sx={stylesLista.linhas}>
                            <Typography gutterBottom>{cliente.observacao}</Typography>
                          </TableCell>
                        )}
                        <TableCell sx={{ borderBottom: '1px solid #959494' }} align="center">
                          <Tooltip title="Editar cliente" arrow>
                            <IconButton color="primary" size="small" onClick={() => handleEditar(cliente.id)}>
                              <EditTwoTone />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Visualizar cliente" arrow>
                            <IconButton sx={{ color: "#424242" }} size="small" onClick={() => handleVisualizar(cliente.id)}>
                              <VisibilityTwoTone />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Deletar cliente" arrow>
                            <IconButton color="error" size="small"onClick={() => handleDelete(cliente.id)}>
                              <DeleteTwoTone />
                            </IconButton>
                          </Tooltip>
                        </TableCell>
