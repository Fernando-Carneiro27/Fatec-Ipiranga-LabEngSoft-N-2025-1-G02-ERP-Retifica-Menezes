...
import { Box, Table, TableBody, TableCell, TableContainer, TableRow, Button, CircularProgress, Typography, TableHead } from '@mui/material';
      ...
      if (id) {
        try {
          ...
          if (estoque.produto?.id) {
            ...
            setProdutoData({ id: produto.id, nome: produto.nome, descricao: produto.descricao, valor: produto.valor });
          }
        } catch (error) {
  ...
  return (
    <HelmetProvider>
      ...
      <Box sx={stylesHistorico.headerHistorico}>
        ...
        />
        <Typography sx={stylesHistorico.headerHistoricoSpan}> Produto - Histórico de Valores de Venda </Typography>
      </Box>

      <Box sx={stylesHistorico.produtosDetalhes}>
        <Box sx={stylesHistorico.abas}>
          <Typography variant="h6" sx={stylesHistorico.aba}> {produtoData.nome} </Typography>
        </Box>
